      ...
      <template slot="extra">
        ...
        >
          <template slot="content">
            <div v-if="iflogin" class="userButton" @click="toBackStage">
              前往后台
            </div>
            <div v-if="iflogin" class="userButton" @click="toMainPage">
              前往首页
            </div>
            <div v-if="iflogin" class="userButton" @click="logout">
              退出登录
            </div>
        ...
        >
          ...
          <a-button type="primary">
            <a-icon type="question" style="color: rgba(0, 0, 0, 0.25)" />
          </a-button>
        </a-popover>
      </template>
    </a-page-header>
...
export default {
  ...
  computed: {
    state: function () {
      return !(
        JSON.stringify(this.common.loginuserinfo) === "{}" ||
        this.common.loginuserinfo == null
      );
    },
    username: function () {
  ...
  methods: {
    ...
    changeback() {
      document
        ...
        .setAttribute(
          "style",
          "background-color:" + this.$store.state.color2 + ";"
        );
    },
    logout: function () {
  ...
  beforeMount() {
    ...
    if (typeof this.$route.params.userid == "undefined") {
      this.$store.commit("changecolorSelect", 1);
      let i = 1;
      this.$store.commit("changecolor", [
        this.$store.state.colorSelect[i].color1,
        1,
      ]);
      this.$store.commit("changecolor", [
        this.$store.state.colorSelect[i].color2,
        2,
      ]);
      this.$store.commit("changecolor", [
        this.$store.state.colorSelect[i].color3,
        3,
      ]);
      this.$store.commit("changecolor", [
        this.$store.state.colorSelect[i].color4,
        4,
      ]);
      this.$store.commit("changecolor", [
        this.$store.state.colorSelect[i].colorFont,
        "Font",
      ]);
      this.$store.commit("changecolor", [
        this.$store.state.colorSelect[i].colorFont2,
        "Font2",
      ]);
      this.changeback();
      return;
    ...
    this.$axios
      .post(this.common.serveraddress + "/user/getbc", params)
      .then((res) => {

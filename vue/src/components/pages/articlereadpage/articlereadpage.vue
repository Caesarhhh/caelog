    ...
    <a-row>
      <a-col :span="18">
        <div id="readcontent" :style="getcolor4()">
          <div id="title">
            <GeminiScrollbar v-if="gemiRefresh>0">{{ title }}</GeminiScrollbar>
          </div>
          <div id="showarea">
            <GeminiScrollbar v-if="gemiRefresh>0">
              <div id="gsd">
      ...
      <div id="commentarea" :style="getcolor1()">
        ...
        <cc class="commentCard clearfix" :datas.sync="commentCardinfo[(pagenumsinfo.pos-1)*3]"
            :replyinfo.sync="replyinfo" :allcomments="commentCardinfo" v-if="(pagenumsinfo.pos-1)*3<commentnum"></cc>
        <cc class="commentCard clearfix" :datas.sync="commentCardinfo[(pagenumsinfo.pos-1)*3+1]"
            :replyinfo.sync="replyinfo" :allcomments="commentCardinfo" v-if="(pagenumsinfo.pos-1)*3+1<commentnum"></cc>
        <cc class="commentCard clearfix" :datas.sync="commentCardinfo[(pagenumsinfo.pos-1)*3+2]"
            :replyinfo.sync="replyinfo" :allcomments="commentCardinfo" v-if="(pagenumsinfo.pos-1)*3+2<commentnum"></cc>
        <pn id="pagenums" :datas.sync="pagenumsinfo"></pn>
        <rtl :add-comment="addaddedcomment" id="replytool" :commentinfo.sync="commentCardinfo" :datas="replyinfo"
             v-if="replyinfo.ifreply"></rtl>
      </div>
    </a-row>
  </div>
</template>
        ...
        this.$axios.get(
          ...
          res => {
            ...
            that.$nextTick(function (){
              ...
              that.mathjax.MathQueue("content")
            })
          })
      },
